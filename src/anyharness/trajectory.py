...
class _SampleBuilder:
    ...
    * **FORK** -- divergence too large or too early to absorb; this builder is
    ...
    def classify_token_drift(self, turn: TurnRecord) -> DriftKind:
        ...
        (heal a divergence inside the most-recent response span) or to FORK
        (the divergence sits too early to absorb, or healing it would cost more
        trained signal than forking). With no drift the turn is handled the CLEAN
        way -- a plain prefix extension.

        REALIGN heals by overwriting the drifted span as ``loss_mask=0``, so its
        cost is exactly the *trained* tokens in that span. Forking costs only
        contiguity: the closed builder keeps its own loss mask and the new one
        starts fresh, so every sampled token still trains, just across two Samples.
        Trained signal is therefore never worth trading for contiguity, and REALIGN
        is allowed only when it destroys none.

        This matters because template drift is not always cosmetic. A chat template
        may render an assistant message differently once it is no longer the final
        message -- Qwen3 emits ``<think>...</think>`` when the message is last and
        strips it in history position -- so the re-rendered prompt diverges across
        the whole response, every turn. Healing that would zero every turn but the
        last, which is exactly the tool-call turns an agentic run needs to train on.
        """
        ...
        # Divergence before the most-recent response span (or an empty builder)
        # can't be absorbed at all: earlier turns are already committed.
        start = self.last_response_start_idx
        if start is None or realign_at < start:
            return DriftKind.FORK

        # Healing would rewrite tokens[realign_at:] as loss_mask=0. Fork rather
        # than destroy trained tokens; only an all-untrained span may REALIGN.
        if any(self.loss_mask[realign_at:]):
            return DriftKind.FORK
